 ...
 */
export class Energy extends Measurement {


  constructor(value?: number, unit?: UnitOfMeasurement);
  constructor(model?: Partial<Energy>);
  constructor(...args: Array<number | UnitOfMeasurement | Partial<Energy> | undefined>) {
    let model: Partial<Energy> = {};
    if (args?.length === 1) {
      model = args[0] as Partial<Energy>;
    }
