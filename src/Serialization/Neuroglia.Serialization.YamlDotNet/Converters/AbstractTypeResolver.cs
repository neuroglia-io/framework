...
namespace Neuroglia.Serialization.YamlDotNet
{
    ...
    public class AbstractTypeResolver
    {
        ...
        public AbstractTypeResolver(Type abstractType)
        {
            ...
            foreach (Type derivedType in TypeCacheUtil.FindFilteredTypes($"nposm:json-polymorph:{abstractType.Name}",
                (t) => t.IsClass && !t.IsAbstract && t.BaseType == abstractType))
            {
        ...
        public bool TryResolve(ParsingEventStream stream, out Type implementationType)
        {
            ...
            }, out Scalar key, out ParsingEvent value))
            {
                if (value is Scalar valueScalar)
                    return this.TypeMappings.TryGetValue(valueScalar.Value, out implementationType);
            }
            return false;
        }
