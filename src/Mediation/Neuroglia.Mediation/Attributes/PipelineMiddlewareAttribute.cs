        ...
        : Attribute
    {
        ...
        public PipelineMiddlewareAttribute(Type pipelineBehaviorType, int priority = 99)
        {
            if (pipelineBehaviorType == null)
                throw new ArgumentNullException(nameof(pipelineBehaviorType));
            if (pipelineBehaviorType.GetGenericType(typeof(IMiddleware<,>)) == null)
                throw new ArgumentException($"The specified type must be implement the '{typeof(IMiddleware<,>).Name}' interface", nameof(pipelineBehaviorType));
