...
namespace Neuroglia.Mediation
{
        ...
        where TResult : IOperationResult
    {
        ...
        {
            if (pipelineExecutionTask.IsFaulted)
                ExceptionDispatchInfo.Capture(pipelineExecutionTask.Exception.InnerException).Throw();
            return (object)pipelineExecutionTask.Result;
        }
