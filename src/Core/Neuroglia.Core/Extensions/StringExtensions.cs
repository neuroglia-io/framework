...
namespace Neuroglia
{
    ...
    public static class StringExtensions
    {
        ...
        public static string GenerateRandomAlphabeticString(int length, StringCase stringCase = StringCase.Lower | StringCase.Upper)
        {
            char[] characters = stringCase switch
            {
                StringCase.Lower | StringCase.Upper => LowerCaseAlphabeticCharacters.ToArray(),
                StringCase.Lower => LowerCaseAlphabeticCharacters.ToArray(),
                StringCase.Upper => UpperCaseAlphabeticCharacters.ToArray(),
        ...
        /// <param name="stringCase">The case of the resulting string</param>
        /// <returns>A new random string of the specified length</returns>
        public static string GenerateRandomAlphanumericString(int length, StringCase stringCase = StringCase.Lower | StringCase.Upper)
        {
            char[] characters = stringCase switch
            {
                StringCase.Lower | StringCase.Upper => (AlphabeticCharacters + NumericCharacters).ToArray(),
                StringCase.Lower => (LowerCaseAlphabeticCharacters + NumericCharacters).ToArray(),
                StringCase.Upper => (UpperCaseAlphabeticCharacters + NumericCharacters).ToArray(),
                _ => throw new NotSupportedException($"The specified {nameof(StringCase)} '{stringCase}' is not supported"),
            };
            byte[] data = new byte[4 * length];
            using (RNGCryptoServiceProvider crypto = new())
